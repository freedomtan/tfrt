...
struct OpAttrsRawEntry final {
  ...
  union {
    ...
    // of an array of elements. IsInlined() must return false.
    const void* data;

    // Buffer that stores the copy of the scalar value. IsInlined() must
    // return true.
    char buffer[sizeof(void*)];
  };

  // This indicates the number of elements in the attribute if it is an array
  // or is a kScalarSentinel if this is a scalar.  This is sized to 56 bits to
  // fit nicely on 64-bit systems.
  ssize_t array_size : (sizeof(void*) == 8 ? 56 : sizeof(void*) * 8);

  // This indicates the type of the entry.
  OpAttrType type : 8;

  // array_size is set to kScalarSentinel if this is a scalar attribute.
  static constexpr ssize_t kScalarSentinel = -1;

  bool IsArray() const { return array_size != kScalarSentinel; }

  bool IsInlined() const {
    return !IsArray() &&
           GetHostSizeAndAlignment(data, type).first <= sizeof(void*);
  }

  // Return the pointer to the underlying data.
  const void* GetData() const { return IsInlined() ? buffer : data; }
...
// out-of-line representation when needed. This makes it efficient in the normal
// case of zero or a few attributes, but supports the general case of many large
// attributes when needed.
//
// OpAttrs differentiates between two different kinds of attributes: scalar and
// array attributes, and the getter will only see the attribute if it is used
// with the corresponding accessor. OpAttrs also has a 'raw' interface which can
// be used for dynamic reflection use-cases and you can also iterate over all of
// the attributes in the set. Iteration order is non-deterministic.
class OpAttrs final {
 public:
  OpAttrs();
  ~OpAttrs();
  ...
  // Set an attribute to the specified value, returning true on success or false
  // if an attribute with the specified name already exists.
  template <typename T>
  ...
  // OpAttrsRawEntry::kScalarSentinel, this sets a scalar entry.  This returns
  // true on success or false if an attribute with that name already exists.
  ...
  // reflection.  Note that this produces the attributes in a non-deterministic
  // order.
  void IterateEntries(
 ...
 private:
  ...
  // Most op invocations have a small number of attributes, and we want them to
  // be formed without an allocation.  This array holds the string and value
  // data and is sized to be large enough to handle the common cases.
  //
  // Note that all of the inline state is undefined if OpAttrs has grown to use
  ...
  // This is the inline representation of the entries index.  Ops should have a
  // small number of attributes, so we just do linear searches through them in
  // lookups.  We automatically switch to an indexed structure when this number
...
// Return the OpAttrType converted from BEFDataType in BEF.
